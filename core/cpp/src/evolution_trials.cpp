...
double critical_value(double confidence_level) noexcept {
    if (confidence_level == 0.75) return 1.150349380376008;
...
bool valid_confidence(double value) noexcept {
    return value == 0.75 || value == 0.80 || value == 0.90 || value == 0.95 ||
...
TrialComparison EvolutionTrials::compare(const TrialStatistics& baseline,
    ...
    if (baseline.count < 2 || candidate.count < 2 || !valid_confidence(confidence_level) ||
        !std::isfinite(baseline.mean) || !std::isfinite(candidate.mean) ||
        !std::isfinite(baseline.variance) || !std::isfinite(candidate.variance)) return result;
    ...
    if (!std::isfinite(se2) || se2 < 0.0) return result;
    if (se2 == 0.0) {
        result.valid = true;
        result.confidence_interval_low = result.mean_difference;
        result.confidence_interval_high = result.mean_difference;
        result.effect_size = result.mean_difference == 0.0 ? 0.0 :
            (result.mean_difference > 0.0 ? std::numeric_limits<double>::infinity()
                                          : -std::numeric_limits<double>::infinity());
        return result;
    }
    ...
    result.effect_size = pooled_variance > 0.0
        ? result.mean_difference / std::sqrt(pooled_variance) : 0.0;
    ...
    result.valid = std::isfinite(result.confidence_interval_low) &&
...
bool EvolutionTrials::supports_adoption(const TrialStatistics& baseline,
    ...
    return comparison.valid && comparison.confidence_interval_low > minimum_gain;
}
