...
namespace { std::uint64_t now_ns() noexcept { return static_cast<std::uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::system_clock::now().time_since_epoch()).count()); } }

Brain::Brain() { const auto history = memory_.all(); for (const auto& event : history) replay(event); state_.events_seen = static_cast<std::uint64_t>(history.size()); if (!history.empty()) state_.cycle = history.back().sequence; }

void Brain::replay(const Event& event) {
    state_.cycle = std::max(state_.cycle, event.sequence);
    if (event.kind == "prediction") {
        const auto key_it = event.data.find("key"), value_it = event.data.find("value");
        if (key_it == event.data.end() || value_it == event.data.end()) return;
        const auto key = std::get_if<std::string>(&key_it->second); if (!key) return;
        double confidence = 0.0; if (const auto it = event.data.find("confidence"); it != event.data.end()) if (const auto p = std::get_if<double>(&it->second)) confidence = *p;
        predictions_[*key] = Prediction{*key, value_it->second, std::clamp(confidence, 0.0, 1.0), event.sequence, false, 0.0}; return;
    }
    ...
    for (const auto& [key, value] : event.data) {
        ...
        else { const bool same = it->second.value == value; it->second.value = value; it->second.confidence = same ? std::min(0.999, it->second.confidence + (1.0 - it->second.confidence) * 0.12) : std::max(0.05, it->second.confidence * 0.82); ++it->second.observations; it->second.updated_sequence = event.sequence; }
        world_.observe(Fact{event.source, key, value, 0.7, 1});
    }
}

Observation Brain::observe(Event event) {
    std::unique_lock lock(mutex_); event.timestamp_ns = event.timestamp_ns == 0 ? now_ns() : event.timestamp_ns;
    const auto previous = memory_.recent(1); const double novelty = compute_novelty(event, previous);
    std::vector<Belief> before; before.reserve(beliefs_.size()); for (const auto& [_, b] : beliefs_) before.push_back(b);
    event.sequence = memory_.append(event); ++state_.cycle; state_.novelty = novelty; replay(event);
    std::vector<Belief> after; after.reserve(beliefs_.size()); for (const auto& [_, b] : beliefs_) after.push_back(b); causal_.observe_transition(before, after);
    double strongest = 0.0; for (const auto& [_, b] : beliefs_) strongest = std::max(strongest, b.confidence);
    attention_state_ = attention_model_.score(event, novelty, strongest); threat_state_ = threat_model_.assess(event);
    state_.attention = attention_state_.salience; state_.threat = threat_state_.score;
    if (const auto it = event.data.find("health"); it != event.data.end()) { double health = 1.0; if (const auto p = std::get_if<double>(&it->second)) health = *p; else if (const auto p = std::get_if<std::int64_t>(&it->second)) health = static_cast<double>(*p); resilience_.observe(event.source, std::clamp(health, 0.0, 1.0)); }
    return Observation{std::move(event), novelty};
}

double Brain::learn(const Evidence& evidence) {
    std::unique_lock lock(mutex_); const double fused = knowledge_.assimilate(evidence); Event event{0, now_ns(), evidence.source, "learning", {{evidence.key, evidence.value}, {"reliability", fused}}}; event.sequence = memory_.append(event); ++state_.events_seen; ++state_.cycle; replay(event); return fused;
}

std::vector<Belief> Brain::beliefs() const { std::shared_lock lock(mutex_); std::vector<Belief> result; result.reserve(beliefs_.size()); for (const auto& [_, b] : beliefs_) result.push_back(b); return result; }
