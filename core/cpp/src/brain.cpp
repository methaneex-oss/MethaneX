...
void Brain::replay(const Event& event) {
    ...
    if (event.kind == "prediction") {
        ...
        if (const auto it = event.data.find("confidence"); it != event.data.end()) if (const auto p = std::get_if<double>(&it->second)) confidence = *p;
    ...
    if (event.kind == "prediction_outcome") {
        ...
        it->second.error = 1.0;
        if (error_it != event.data.end()) if (const auto p = std::get_if<double>(&error_it->second)) it->second.error = std::clamp(*p, 0.0, 1.0);
        return;
    }
    ...
    if (event.kind == "learning") {
        if (const auto it = event.data.find("reliability"); it != event.data.end()) if (const auto p = std::get_if<double>(&it->second)) reliability = std::clamp(*p, 0.0, 1.0);
    }

    for (const auto& [key, value] : event.data) {
        if (event.kind == "learning" && key == "reliability") continue;
        auto it = beliefs_.find(key);
        if (it == beliefs_.end()) beliefs_.emplace(key, Belief{key, value, reliability, 1, event.sequence});
        else {
            const bool same = it->second.value == value;
            it->second.value = value;
            it->second.confidence = same ? std::min(0.999, it->second.confidence + (1.0 - it->second.confidence) * reliability) : std::max(0.05, it->second.confidence * (1.0 - reliability));
...
bool Brain::resolve_prediction(const std::string& key, const Scalar& actual) {
    ...
    if (const auto predicted = std::get_if<double>(&it->second.predicted)) if (const auto observed = std::get_if<double>(&actual)) adaptation_.observe(key, *predicted, *observed);
