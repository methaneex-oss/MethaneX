...
MemoryTier Memory::tier_of(const Event& event) const noexcept {
    ...
    const auto best = std::max_element(it->second.begin(), it->second.end());
    return static_cast<MemoryTier>(std::distance(it->second.begin(), best));
}
...
std::vector<MemoryRecord> Memory::recall_ranked(const Attributes& query, std::size_t limit) const {
    ...
    for (std::size_t index = 0; index < metadata_.size(); ++index) {
        ...
        const double components[] = {
            std::max(matches / denominator, std::numeric_limits<double>::min()),
            std::max(overlap / denominator, std::numeric_limits<double>::min()),
