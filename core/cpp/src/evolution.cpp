...
bool EvolutionModel::rollback(const std::string& key) { auto it = parameters_.find(key); if (it == parameters_.end()) return false; const double old = it->second.value; it->second.value = it->second.baseline; it->second.previous_value = it->second.baseline; return old != it->second.value; }
