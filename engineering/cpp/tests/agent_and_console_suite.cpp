...
int main(){
Builder builder; EngineeringCoordinator c; DirectExecutionBoundary boundary; AllowAllAuthorizer auth; EngineeringTask task{"task-1","implement change",{"code.edit"},{"workspace.write"},{"source"},{"patch"},AgentRisk::medium,1.0}; auto ranked=c.rank(task,{builder.descriptor()}); assert(ranked.size()==1&&ranked.front().eligible); auto result=c.dispatch(task,builder,auth,boundary); assert(result.accepted);
AgentMessageBus bus; EngineeringOperatorConsole console(bus,"operator","run-1","workspace-1",[](const std::string&s,const std::string&r,AgentMessageType t){return s=="operator"&&r=="builder"&&t==AgentMessageType::request;}); AgentCommunicationEndpoint peer(bus,"builder","run-1","workspace-1"); assert(console.connected()&&peer.registered());
EngineeringOperatorDialogue dialogue(console); auto sent=dialogue.send("builder","task-1","inspect and build"); assert(sent.accepted); auto incoming=peer.drain(); assert(incoming.size()==1&&incoming.front().sender_id=="operator"&&incoming.front().payload=="inspect and build"); auto reply=peer.send("m2","operator","task-1",AgentMessageType::result,"build complete"); assert(reply.accepted); auto received=dialogue.receive(); assert(received.size()==1&&received.front().sender_id=="builder"&&received.front().payload=="build complete"); assert(dialogue.transcript().size()==2); return 0; }
