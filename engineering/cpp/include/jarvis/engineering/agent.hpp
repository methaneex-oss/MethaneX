...
struct AgentArtifact { std::string type; std::string location; std::string digest; };
struct AgentEvidence { std::string kind; std::string value; };
struct AgentResult {
    bool accepted{false}; std::string agent_id; std::string task_id; std::string reason;
    std::vector<AgentArtifact> artifacts; std::vector<AgentEvidence> evidence;
};
