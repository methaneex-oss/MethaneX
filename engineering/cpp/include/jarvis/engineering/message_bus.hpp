...
// Human-facing bridge to the same communication fabric used by engineering agents.
// It deliberately contains no model/provider logic: an external UI, CLI, API, or
// ChatGPT bridge can translate operator input into messages and render responses.
class AgentOperatorEndpoint {
public:
    AgentOperatorEndpoint(
        AgentMessageBus& bus,
    ...
    AgentOperatorEndpoint(const AgentOperatorEndpoint&) = delete;
    AgentOperatorEndpoint& operator=(const AgentOperatorEndpoint&) = delete;
