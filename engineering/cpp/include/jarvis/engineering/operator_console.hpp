...
struct OperatorConsolePolicy {
    std::size_t maximum_pending_messages{256};
    bool allow_requests{true};
    bool allow_queries{true};
    bool allow_feedback{true};
};

class EngineeringOperatorConsole {
public:
    ...
    ~EngineeringOperatorConsole();
    EngineeringOperatorConsole(const EngineeringOperatorConsole&) = delete;
    EngineeringOperatorConsole& operator=(const EngineeringOperatorConsole&) = delete;
...
private:
    bool allowed(AgentMessageType type) const noexcept;

    AgentCommunicationEndpoint endpoint_;
    Authorization authorization_;
    OperatorConsolePolicy policy_;
};
