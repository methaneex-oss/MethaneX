...
public:
    OpenAIAstraModelProvider(OpenAIAstraModelConfig config, HttpTransport transport) noexcept;
...
private:
    ...
    static std::string build_request_body(const OpenAIAstraModelConfig& config, const ModelRequest& request);
    static std::string extract_json_string(std::string_view json, std::string_view key, std::size_t from = 0);
    static std::vector<ModelFileChange> extract_file_changes(std::string_view json);
