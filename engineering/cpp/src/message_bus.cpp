...
MessageBusResult AgentCommunicationEndpoint::send(
    ...
    AgentMessage message{
        std::move(message_id), 0, run_id_, workspace_id_, agent_id_,
        std::move(recipient_id), std::move(correlation_id), type,
        std::move(payload)};
...
AgentOperatorEndpoint::AgentOperatorEndpoint(
    AgentMessageBus& bus,
    ...
    : bus_(bus),
      ...
      workspace_id_(std::move(workspace_id)),
      maximum_pending_messages_(maximum_pending_messages) {
    if (operator_id_.empty() || run_id_.empty() ||
        maximum_pending_messages_ == 0) {
        return;
    }
...
bool AgentOperatorEndpoint::registered() const noexcept {
    return registered_;
}

const std::string& AgentOperatorEndpoint::operator_id() const noexcept {
    return operator_id_;
}

MessageBusResult AgentOperatorEndpoint::send_to_agent(
    std::string message_id,
    std::string agent_id,
    std::string correlation_id,
    AgentMessageType type,
    std::string payload) {
    if (!registered_) {
        return {false, 0, "operator endpoint is not registered"};
    }

    AgentMessage message{
        std::move(message_id), 0, run_id_, workspace_id_, operator_id_,
        std::move(agent_id), std::move(correlation_id), type,
        std::move(payload)};
    return bus_.send(std::move(message));
}

void AgentOperatorEndpoint::receive_message(const AgentMessage& message) {
    if (message.run_id != run_id_ ||
        (!workspace_id_.empty() && message.workspace_id != workspace_id_)) {
        return;
    }
...
std::vector<AgentMessage> AgentOperatorEndpoint::receive() {
    std::lock_guard lock(mutex_);
    std::vector<AgentMessage> messages;
    messages.reserve(pending_messages_.size());
    while (!pending_messages_.empty()) {
...
std::size_t AgentOperatorEndpoint::pending() const noexcept {
    std::lock_guard lock(mutex_);
    return pending_messages_.size();
}
