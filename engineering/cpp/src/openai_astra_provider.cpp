...
constexpr std::string_view kSchema =
    R"JSON({
      ...
      "required":["summary","file_changes","evidence"],
      "additionalProperties":false
    })JSON";

std::size_t find_matching_object(std::string_view json, std::size_t start) {
...
ModelDescriptor OpenAIAstraModelProvider::descriptor() const {
    return {
        "openai-astra",
        "openai",
        config_.model,
...
bool OpenAIAstraModelProvider::valid_config(
    const OpenAIAstraModelConfig& config) noexcept {
...
std::string OpenAIAstraModelProvider::escape_json(std::string_view value) {
    std::string escaped;
...
std::string OpenAIAstraModelProvider::build_request_body(
...
std::vector<ModelFileChange> OpenAIAstraModelProvider::extract_file_changes(
    ...
    while (cursor < json.size() && json[cursor] != ']') {
        ...
        const std::size_t object_end = find_matching_object(json, object_begin);
        ...
        const std::string path = extract_json_string(object, "path", 0);
        const std::string content = extract_json_string(object, "content", 0);
        if (path.empty()) {
            break;
        }
...
ModelResponse OpenAIAstraModelProvider::generate(const ModelRequest& request) {
    if (!valid_config(config_) || !valid_model_request(request) || !transport_) {
        return {ModelProviderStatus::rejected, "openai", config_.model,
    ...
    if (response.status_code == 401 || response.status_code == 403) {
        return {ModelProviderStatus::rejected, "openai", config_.model,
                {}, {}, {}, "openai authorization rejected"};
    }
    if (response.status_code == 408 || response.status_code == 429 ||
        response.status_code >= 500) {
        return {ModelProviderStatus::unavailable, "openai", config_.model,
                {}, {}, {}, response.error.empty()
                    ? "openai temporarily unavailable"
                    : response.error};
    }
    if (response.status_code < 200 || response.status_code >= 300) {
        return {ModelProviderStatus::failed, "openai", config_.model,
                {}, {}, {}, response.error.empty()
                    ? "openai request failed"
                    : response.error};
    }

    const std::string output = extract_json_string(response.body, "text", 0);
    if (output.empty()) {
        return {ModelProviderStatus::failed, "openai", config_.model,
    ...
    if (output.size() > request.maximum_output_bytes) {
        return {ModelProviderStatus::failed, "openai", config_.model,
                {}, {}, {}, "openai output limit exceeded"};
    }

    const auto file_changes = extract_file_changes(output);
    return {ModelProviderStatus::succeeded, "openai", config_.model,
            output, file_changes,
            {{"model.provider", "openai"},
             {"model.id", config_.model},
             {"model.reasoning_effort", config_.reasoning_effort}},
            {}};
}
