...
class PersistentMemory:
    ...
    def __init__(self, path: str | Path = "data/jarvis_memory.db") -> None:
        self.path = Path(path)
        self.path.parent.mkdir(parents=True, exist_ok=True)
        self.db: sqlite3.Connection | None = sqlite3.connect(self.path)
        self.db.execute(
            "CREATE TABLE IF NOT EXISTS memories ("
            "id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT NOT NULL, "
            "tier INTEGER NOT NULL, confidence REAL NOT NULL, source TEXT NOT NULL, "
    ...
    def remember(self, content: str, tier: int = 3, confidence: float = 1.0, source: str = "user") -> Memory:
        ...
        if tier == 1 and source != "system":
            raise PermissionError("Tier 1 is immutable system identity")
        ...
        cur = self.db.execute(
            "INSERT INTO memories(content,tier,confidence,source,created_at) VALUES(?,?,?,?,?)",
            (content, tier, confidence, source, now),
        )
        self.db.commit()
    ...
    def search(self, query: str, limit: int = 5) -> list[Memory]:
        ...
        rows = self.db.execute(
            "SELECT id,content,tier,confidence,source,created_at FROM memories"
        ).fetchall()
